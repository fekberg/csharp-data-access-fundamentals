...
supportRequestRepository.Add(
    customerId, "filip@ekberg.dev", "Hello Pluralsight!"
);

foreach(var request 
    in supportRequestRepository.Get(customerId))
{
...
foreach (var item in itemRepository.All())
{
    var details = 
        itemDetailService.GetDetailsFor(item.Id.ToString());
